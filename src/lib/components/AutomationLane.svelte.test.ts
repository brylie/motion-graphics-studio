...
describe('AutomationLane', () => {
	...
	describe('keyframe positioning', () => {
		it('positions keyframes correctly based on time and pixelsPerSecond', () => {
			...
			expect((visibleKeyframes[0] as SVGCircleElement).getAttribute('cx')).toBe(
				'0'
			);

			// Second keyframe at time 2.5: 2.5 * 100 = 250px
			expect((visibleKeyframes[1] as SVGCircleElement).getAttribute('cx')).toBe(
				'250'
			);

			// Third keyframe at time 5: 5 * 100 = 500px
			expect((visibleKeyframes[2] as SVGCircleElement).getAttribute('cx')).toBe(
				'500'
			);
		});
		...
		it('positions keyframes within clip duration bounds', () => {
			...
			const firstX = parseFloat(
				(visibleKeyframes[0] as SVGCircleElement).getAttribute('cx') || '0'
			);
			const lastX = parseFloat(
				(visibleKeyframes[1] as SVGCircleElement).getAttribute('cx') || '0'
			);
		...
		it('inverts Y values correctly (value 0 at bottom, 1 at top)', () => {
			...
			const cy0 = parseFloat(
				(visibleKeyframes[0] as SVGCircleElement).getAttribute('cy') || '0'
			);
			const cy1 = parseFloat(
				(visibleKeyframes[1] as SVGCircleElement).getAttribute('cy') || '0'
			);
			const cy2 = parseFloat(
				(visibleKeyframes[2] as SVGCircleElement).getAttribute('cy') || '0'
			);
